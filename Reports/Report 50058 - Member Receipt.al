report 50058 "Member Receipt"
{
    ...
    DefaultLayout = RDLC;
    RDLCLayout = './EPCReports/Member Receipt.rdl';
    ...
    dataset
    {
        dataitem("New Confirmed Order"; "New Confirmed Order")
        {
            ...
            column(No; "New Confirmed Order"."No.")
            {
            }
            column(BarcodeScan; BarcodeConverter("New Confirmed Order"."No.")) // '*' + "New Confirmed Order"."No." + '*'
            {
            }
            ...
            column(SaleableArea_NewConfirmedOrder; FORMAT("New Confirmed Order"."Saleable Area") + ' Sqd ')
            {
            }
            column(UmasterFacing; Umaster.Facing)
            {
            }
            column(TotalReceivedAmount_NewConfirmedOrder; FORMAT("New Confirmed Order"."Total Received Amount"))
            {
            }
            ...
            dataitem("NewApplication Payment Entry"; "NewApplication Payment Entry")
            {
                ...
                begin
                    ...
                    IF "NewApplication Payment Entry"."Deposit/Paid Bank" <> '' THEN
                        BankAccount.CHANGECOMPANY("New Confirmed Order"."Company Name");
                    IF BankAccount.GET("NewApplication Payment Entry"."Deposit/Paid Bank") THEN;
                    FooterDesc := '';

                    IF "New Confirmed Order"."Company Name" = 'BBG India Developers LLP' THEN BEGIN
                    ...
                    END ELSE BEGIN
                        RespCenters_1.RESET;
                        IF RespCenters_1.GET("New Confirmed Order"."Shortcut Dimension 1 Code") THEN BEGIN
            ...
            begin
                ...
                Clear(RecRef);
                BarcodeConverter2D("New Confirmed Order"."No.");
                // RecRef.GetTable("New Confirmed Order");
                // TempBlob.ToRecordRef(RecRef, "New Confirmed Order".FieldNo(Barcode));
                "New Confirmed Order".Modify();
                "New Confirmed Order".CalcFields(Barcode);
                CapPlot := '';
                PlotNo := '';
                THeadName := '';
                THeadPHNo := '';
                ...
                IF RespCenter.GET("Shortcut Dimension 1 Code") THEN BEGIN
                    ...
                    IF NOT RespCenter."Fields Not Show on Receipt" THEN BEGIN
                        PlotCost := 'Plot Cost';
                        PlotAmt := "New Confirmed Order".Amount;
                        CapDueAmt := 'Amount Due';
                        DueAmt := Amount - ABS("Total Received Amount");
                    END;
                END;
                ...
                //IF NOT CurrReport.PREVIEW THEN BEGIN  //Code commented 25112025
                InsertPrintLog('Member Receipt', "Application No.");
                //END;  //Code commented 25112025
                ...
                */ //040517 code comment
                ...
                RegionwiseVendor_1.SETFILTER("No.", "Introducer Code");
                ...
                AsswithApp.RESET;
                //  AsswithApp.CHANGECOMPANY("Company Name");
                AsswithApp.SETRANGE("Application Code", "No.");
        ...
        dataitem(Integer; Integer)
        {
            ...
            begin
                RespCenter.RESET;
                IF RespCenter.GET("New Confirmed Order"."Shortcut Dimension 1 Code") THEN
    ...
    var
        ...
        SkipRow: Integer;
        PlotCaption: Text;
        R6628: Report "Lot No Label";
        QR: Codeunit "QR Generator";
