page 50352 "Regular Gold/Silver Voucher"
{
    ...
    layout
    {
        area(content)
        {
            group(General)
            {
                ...
                field("Application No."; Rec."Application No.")
                {
                    ...
                    begin
                        ...
                        Rec.TESTFIELD("Item Type", Rec."Item Type"::Silver);
                        NewConfirmedOrder.RESET;
                        IF NewConfirmedOrder.GET(Rec."Application No.") THEN BEGIN
                            CLEAR(CustomFunctions);
                            Rec."Silver Gram Eligibility" := CustomFunctions.BalancSilverGramEligibility(NewConfirmedOrder);
                        END ELSE
                            Rec."Silver Gram Eligibility" := 0;
                    end;
                }
                field("Silver Gram Eligibility"; Rec."Silver Gram Eligibility")
                {
                    Editable = false;
                }
            ...
            part("MIN Lines"; "Gold_Silver Voucher Lines-R")
            {
    ...
    actions
    {
        area(navigation)
        {
            ...
            group("&GIN")
            {
                ...
                action("Get Gold/Silver Coins Lines")
                {
                    Caption = 'Get Gold/Silver Voucher Lines';
                    Image = GetLines;
                    Promoted = true;
                    ...
                    begin
                        ...
                        IF GatePassLines.FINDFIRST THEN
                            ERROR('Please delete the below Entries first');

                        IF Rec."Item Type" = Rec."Item Type"::Gold_SilverVoucher THEN BEGIN
                            ...
                            //GoldCoinApproval.SETRANGE("Item Type", GoldCoinApproval."Item Type"::Gold);
                ...
                action("Sent for Approval")
                {
                    ...
                    begin
                        ...
                        // CheckSilverGram; //10042025 Code commented
            ...
            group("&Post")
            {
                ...
                action("Post")
                {
                    ...
                    begin
                        ...
                        MemberOf.RESET;
                        MemberOf.SETRANGE(MemberOf."User Name", USERID);
                        MemberOf.SETRANGE(MemberOf."Role ID", 'A_GOLDCOINISSUE');
                        IF NOT MemberOf.FINDFIRST THEN
                            ERROR('You do not have permission of role :A_GOLDCOINISSUE');
                        //ALLECK 060313 End
                        ...
                        IF CONFIRM(txtConfirm, TRUE) THEN BEGIN
                            ...
                            //UpdateGoldGenerate;  //10042025 code commented.
                            ...
                            //                 SMSLogDetails.SMSValue(SmsMessage, SmsMessage1, 'Customer', Customer."No.", Customer.Name, 'Gold Issue', Rec."Shortcut Dimension 1 Code", GetDescription.GetDimensionName(Rec."Shortcut Dimension 1 Code", 1), APPNo);
                            //                 //ALLEDK15112022 END
    ...
    var
        ...
        GoldCoinApprovalPAGE: Page "Get Gold/Silver Voucher";
        CustomerNo: Code[20];
    ...
    //                         ELSE IF Rec."Item Type" = Rec."Item Type"::Silver THEN
    //                             ConfOrder."Silver Coin Generated" := TRUE;
    //                         ConfOrder.MODIFY;
    //                     END;
    //                 END;
    //             END;
    ...
    // local procedure CheckSilverGram()
    // var
    //     RecGatePassLine: Record "Gate Pass Line";
    //     TotalGrams: Decimal;
    // begin
    //     IF Rec."Item Type" = Rec."Item Type"::Gold_SilverVoucher THEN BEGIN

    //         RecGatePassLine.RESET;
    //         RecGatePassLine.SETRANGE("Document Type", Rec."Document Type");
    //         RecGatePassLine.SETRANGE("Document No.", Rec."Document No.");
    //         RecGatePassLine.SETFILTER(Qty, '>%1', 0);
    //         IF RecGatePassLine.FINDSET THEN
    //             REPEAT
    //                 TotalGrams := TotalGrams + (RecGatePassLine."Silver / Gold in Grams" * RecGatePassLine.Qty);
    //             UNTIL RecGatePassLine.NEXT = 0;
    //         IF TotalGrams > Rec."Silver Gram Eligibility" THEN
    //             ERROR('Silver Eligibility mismatch');
    //     END;
    // end;
}
