page 50510 "Direct 194R Gift Issue"

{
    ...
    Caption = 'Direct 194R Gift Issue';
    PageType = Card;
    ...
    layout
    {
        area(content)
        {
            ...
            part("MIN Lines"; "194R_Gift Voucher Lines")
            {
            ...
            group("&Post")
            {
                ...
                action("Post")
                {
                    ...
                    begin
                        ...
                        unitsetup.TestField("Perquisite to Associates Exp.");
                        Unitsetup.TestField("TDS Nature of Deducation 194R");
                        unitsetup.TestField("Gift Inventory Account");
                        Rec.TESTFIELD("Approval Status", Rec."Approval Status"::Approved);  //270923 Approval Workflow
                        DocNo := '';
    ...
    begin
        ...
        Rec."Item Type" := Rec."Item Type"::R194_Gift;  //09042025 Code added
        RecRespCenter.RESET;
        RecRespCenter.SETRANGE(Code, Rec."Responsibility Center");
        IF RecRespCenter.FIND('-') THEN BEGIN
            Respname := RecRespCenter.Name;
    ...
    var
        ...
        GenJnlLine_2: Record "Gen. Journal Line";
    ...
    begin
        ...
        ItemJnl."R194_Application No." := RecGatePassLines."R194_Application No.";  //050525 New code added
        ItemJnl."External Document No." := Rec."Document No.";
    ...
    procedure InsertAndPostGnLLines()
    var
        LineNo: Integer;
        AccNo: Code[20];
        Unitsetup: Record "unit setup";
    ...
    begin
        ...
        GPassLine.RESET;
        GPassLine.SETRANGE("Document Type", Rec."Document Type");
        GPassLine.SETRANGE("Document No.", Rec."Document No.");
        ...
        IF GPassLine.FIND('-') THEN
            REPEAT
                ...
                IF TotalAmount > 0 THEN BEGIN
                    ...
                    GenJnlLine."Line No." := vLineNo + 10000;
                    ...
                    GenJnlLine.Validate("Bal. Account No.", Unitsetup."Gift Inventory Account");
                    ...
                    GenJnlLine."External Document No." := Rec."Document No." + '-' + Format(GPassline."Line No." / 10000);
                    GenJnlLine."Ref. Invoice No." := GPassLine."Ref. Invoice No.";
                    vLineNo := GenJnlLine."Line No.";
                    GenJnlLine.MODIFY;
    ...
    procedure InsertGnLLinesWithTDS()
    var
        LineNo: Integer;
        AccNo: Code[20];
        Unitsetup: Record "unit setup";
        ...
        IF GPassLine.FIND('-') THEN
            REPEAT
                ...
                IF TotalAmount > 0 THEN BEGIN
                    ...
                    // TDSP := CalculateTDSPercentage(Rec."Vendor No.");   //Code commented 04082025

                    //TDSBaseAmount := ROUND((100 * TotalAmount) / (100 - TDSP), 0.01);  //Code commented 04082025
                    TDSBaseAmount := ROUND((TotalAmount));// / (1 + TDSP / 100), 0.01);  //Code commented 04082025

                    GenJnlLine_2.INIT;
                    GenJnlLine_2."Journal Template Name" := GenTemplateCode;
                    GenJnlLine_2."Journal Batch Name" := GenBatchCode;
                    GenJnlLine_2."Line No." := vLineNo + 10000;
                    GenJnlLine_2."Posting Date" := Rec."Posting Date";
                    GenJnlLine_2."Document No." := DocNo; //Rec."Document No.";
                    GenJnlLine_2."Document Date" := Rec."Document Date";
                    GenJnlLine_2."Ref Document Type" := GenJnlLine_2."Ref Document Type"::Order;

                    GenJnlLine_2."Document Type" := GenJnlLine_2."Document Type"::Invoice;
                    GenJnlLine_2."Source Code" := GenJnlTemplate."Source Code";
                    GenJnlLine_2."Reason Code" := GenJnlBatch."Reason Code";
                    GenJnlLine_2."Posting Type" := GenJnlLine_2."Posting Type"::Running;
                    //GenJnlLine_2."System-Created Entry" := TRUE;
                    //GenJnlLine_2."External Document No." := Rec."Document No.";
                    GenJnlLine_2."External Document No." := Rec."Document No." + '-' + Format(GPassline."Line No." / 10000);

                    GenJnlLine_2.Validate(Verified, True);
                    GenJnlLine_2."Created By" := USERID;
                    GenJnlLine_2.Validate("Account Type", GenJnlLine_2."Account Type"::Vendor);
                    GenJnlLine_2.VALIDATE("Account No.", Rec."Vendor No.");
                    GenJnlLine_2.VALIDATE(Amount, -1 * TDSBaseAmount);
                    GenJnlLine_2.Validate("Location Code", Rec."Location Code");
                    GenJnlLine_2.Validate("TDS Section Code", Unitsetup."TDS Nature of Deducation 194R");

                    GenJnlLine_2.Validate("Bal. Account Type", GenJnlLine_2."Bal. Account Type"::"G/L Account");
                    GenJnlLine_2.Validate("Bal. Account No.", Unitsetup."Perquisite to Associates Exp.");
                    GenJnlLine_2.VALIDATE(GenJnlLine_2."Shortcut Dimension 1 Code", Rec."Shortcut Dimension 1 Code");
                    GenJnlLine_2."Shortcut Dimension 2 Code" := Rec."Shortcut Dimension 2 Code";
                    GenJnlLine_2."Ref. Invoice No." := GPassLine."Ref. Invoice No.";
                    vLineNo := GenJnlLine_2."Line No.";


                    // GenJnlLine_2."Gen. Bus. Posting Group" := '';
                    // GenJnlLine_2."Gen. Prod. Posting Group" := '';
                    GenJnlLine_2.INSERT;
                    CalculateTax.CallTaxEngineOnGenJnlLine(GenJnlLine_2, GenJnlLine_2);  //190225

                    GEnJnlPostLine.RunWithCheck(GenJnlLine_2);  //070525
                    GenJnlLine_2.DELETE(TRUE);
                END;
    ...
    procedure InsertGnLLinesForAdjustment()
    var
        LineNo: Integer;
        AccNo: Code[20];
        Unitsetup: Record "unit setup";
        ...
        IF GPassLine.FIND('-') THEN
            REPEAT
                ...
                IF TotalAmount > 0 THEN BEGIN
                    ...
                    GenJnlLine_2."Posting Type" := GenJnlLine_2."Posting Type"::Running;
                    //GenJnlLine_2."System-Created Entry" := TRUE;
                    //GenJnlLine_2."External Document No." := DocNo + '-01';
                    GenJnlLine_2."External Document No." := Rec."Document No." + '-' + Format(GPassline."Line No." / 10000);
                    GenJnlLine_2.Validate(Verified, True);
                    GenJnlLine_2."Created By" := USERID;
                    GenJnlLine_2.Validate("Account Type", GenJnlLine_2."Account Type"::"G/L Account");
                    GenJnlLine_2.VALIDATE("Account No.", Unitsetup."Gift Control A/c");
                    GenJnlLine_2.VALIDATE(Amount, TotalAmount);
                    GenJnlLine_2.Validate("Location Code", Rec."Location Code");
                    GenJnlLine_2.Validate("Bal. Account Type", GenJnlLine_2."Bal. Account Type"::"G/L Account");
                    GenJnlLine_2.Validate("Bal. Account No.", Unitsetup."Perquisite to Associates Exp.");
                    GenJnlLine_2.VALIDATE(GenJnlLine_2."Shortcut Dimension 1 Code", Rec."Shortcut Dimension 1 Code");
    ...
    begin
        ...
        RecGatePassLines.SETRANGE("Document No.", Rec."Document No.");
        RecGatePassLines.SETRANGE("Journal Line Created", FALSE);
        IF RecGatePassLines.FINDFIRST THEN begin
            REPEAT
                InsertItemJournals;
            UNTIL RecGatePassLines.NEXT = 0;
            InsertAndPostGnLLines();
            InsertGnLLinesWithTDS();
            InsertGnLLinesForAdjustment();
        END;
        // BBG1.01 ALLE_NB 16102012 End
    end;
    ...
    procedure CalculateTDSPercentage(VendCode: code[20]): Decimal
    var
        TDSPercent: Decimal;
    ...
    begin
        ...
        AllowedSection.SetRange("TDS Section", UnitSetup."TDS Nature of Deducation 194R");
