table 50017 "NewApplication Payment Entry"
{
    ...
    Caption = 'NewApplication Payment Entry';
    ...
    fields
    {
        ...
        field(7; Amount; Decimal)
        {
            ...
            begin
                //ALLETDK061212
                UnitSetup.GET;
                UnitSetup.TESTFIELD("Cash Amount Limit");
                TotalCashAmount := 0;
                v_NewApplicationPaymentEntry.RESET;
                v_NewApplicationPaymentEntry.SETRANGE(v_NewApplicationPaymentEntry."Document No.", "Document No.");
                v_NewApplicationPaymentEntry.SETRANGE(Posted, TRUE);
                v_NewApplicationPaymentEntry.SETFILTER(v_NewApplicationPaymentEntry."Cheque Status", '%1|%2', v_NewApplicationPaymentEntry."Cheque Status"::" ", v_NewApplicationPaymentEntry."Cheque Status"::Cleared);
                v_NewApplicationPaymentEntry.SETRANGE("Payment Mode", v_NewApplicationPaymentEntry."Payment Mode"::Cash);
                IF v_NewApplicationPaymentEntry.FINDSET THEN
                    REPEAT
                        TotalCashAmount := TotalCashAmount + v_NewApplicationPaymentEntry.Amount;
                    UNTIL v_NewApplicationPaymentEntry.NEXT = 0;


                IF "Payment Mode" = "Payment Mode"::Cash THEN BEGIN
                    IF (TotalCashAmount + Amount) > UnitSetup."Cash Amount Limit" THEN
                        ERROR('You can not post cash receipt more than =' + FORMAT(UnitSetup."Cash Amount Limit" - TotalCashAmount));
                END;
                AJVMCode := '';
                ...
                IF APPNo.FINDFIRST THEN BEGIN
                    ...
                    IF "Payment Mode" = "Payment Mode"::AJVM THEN
                        ERROR('Associate to Member transfer Entry can not be done from here');
                END ELSE IF ConfirmOrder.GET("Document No.") THEN BEGIN
                    ConfirmOrder.TESTFIELD(ConfirmOrder."Application Closed", FALSE);  //190820
                    IF ConfirmOrder.Status = ConfirmOrder.Status::Cancelled THEN
                        ERROR('Application already Cancelled');
                ...
                IF "Payment Mode" = "Payment Mode"::AJVM THEN BEGIN
                    ...
                    IF Vendor.GET(AJVMCode) THEN BEGIN
                        ...
                        Vendor.CALCFIELDS("BBG Commission Amount Qualified");
                        Vendor.CALCFIELDS("BBG Travel Amount Qualified");
                        Vendor.CALCFIELDS("BBG Incentive Amount Qualified");
                    END;
                    ...
                    IF (Vendor."BBG Commission Amount Qualified" + Vendor."BBG Travel Amount Qualified" +
                          Vendor."BBG Incentive Amount Qualified" - Vendor."BBG Balance at Date (LCY)") > 0 THEN BEGIN
                        IF "Associate Transfer Amount" > ABS(Vendor."BBG Commission Amount Qualified" + Vendor."BBG Travel Amount Qualified" +
                            Vendor."BBG Incentive Amount Qualified" - Vendor."BBG Balance at Date (LCY)") THEN BEGIN
        ...
        field(15; "Payment Mode"; Option)
        {
            ...
            begin
                ...
                //"User Branch Code" := UserSetup."User Branch";  090921
                "User Branch Code" := UserSetup."Responsibility Center"; //090921
                //ALLETDK141212..END
                ...
                AppPayEntry.RESET;
                AppPayEntry.SETRANGE("Document No.", "Document No.");
                AppPayEntry.SETFILTER("Payment Mode", '%1|%2', AppPayEntry."Payment Mode"::"Refund Cash", AppPayEntry."Payment Mode"::"Refund Bank");
                AppPayEntry.SETRANGE(Posted, TRUE);
                //IF AppPayEntry.FINDFIRST THEN
                //  ERROR('This application has Refund Entry. So you can not post receipt or other transction');

                //ALLE280415


                IF ("Payment Mode" = "Payment Mode"::"Refund Cash") OR ("Payment Mode" = "Payment Mode"::"Refund Bank") THEN BEGIN
                    ...
                    IF NOT Memberof.FINDFIRST THEN BEGIN
                        ...
                        IF AppPayEntry.FINDFIRST THEN BEGIN
                            ERROR('Refund Entry already exist with amount ' + FORMAT(AppPayEntry.Amount));  //1008
                        END;
                    END;
                    //ALLE280415
            ...
            begin
                IF "Bank Type" <> "Bank Type"::ProjectCompany THEN BEGIN
                    ...
                    IF ("Payment Mode" = "Payment Mode"::"Refund Bank") THEN BEGIN
                        ...
                        //VoucherAccount.SETRANGE("Location code",UserSetup."User Branch");  //090921
                        CreditVoucherAccount.SETRANGE("Location code", UserSetup."Responsibility Center");  //090921
                        ...
                        IF CreditVoucherAccount.FINDFIRST THEN BEGIN
                            IF PAGE.RUNMODAL(Page::"Voucher Posting Credit Account", CreditVoucherAccount) = ACTION::LookupOK THEN BEGIN //98019
                    ...
                    IF ("Payment Mode" = "Payment Mode"::Bank) THEN BEGIN
                        ...
                        //VoucherAccount.SETRANGE("Location code",UserSetup."User Branch");   //090921
                        DebitVoucherAccount.SETRANGE("Location code", UserSetup."Responsibility Center");  //090921
                        ...
                        IF DebitVoucherAccount.FINDFIRST THEN BEGIN
                            IF PAGE.RUNMODAL(Page::"Voucher Posting Debit Accounts", DebitVoucherAccount) = ACTION::LookupOK THEN BEGIN //98019
                ...
                END ELSE BEGIN
                    ...
                    TESTFIELD(Posted, FALSE);
                    IF "Document Type" = "Document Type"::Application THEN BEGIN
                        IF RecApplication.GET("Document No.") THEN
                            CompName := RecApplication."Company Name";
                    END;
                    IF "Document Type" = "Document Type"::BOND THEN BEGIN
                        IF RecConfOrder.GET("Document No.") THEN BEGIN
                            CompName := RecConfOrder."Company Name";
                            // IF RecConfOrder."Application Type" = RecConfOrder."Application Type"::Trading THEN
                            //TESTFIELD("Bank Type","Bank Type"::Projectcompany)
                            //TESTFIELD("Bank Type","Bank Type"::CollectionCompany);
                        END;
                    END;
                    UserSetup.GET(USERID);
                    IF ("Payment Mode" = "Payment Mode"::"Refund Bank") THEN BEGIN
        ...
        field(50005; Approved; Boolean)
        {
            ...
            begin
                IF "Payment Mode" = "Payment Mode"::AJVM THEN BEGIN
                    Memberof.RESET;
                    Memberof.SETRANGE("User Name", USERID);
                    Memberof.SETFILTER("Role ID", '%1', 'AJVM');
        ...
        field(50015; "Adjmt. Line No."; Integer)
        {
            ...
            TableRelation = IF ("Payment Mode" = CONST("Negative Adjmt.")) "NewApplication Payment Entry"."Line No." WHERE("Document No." = FIELD("Document No."),
        ...
        field(50206; "Receipt post on InterComp"; Boolean)
        {
            Editable = true;
        }
        ...
        field(50209; "Commmission Reverse"; Boolean)
        {
            Editable = false;
        }
        ...
        field(50224; "MJV Entry Not Posted"; Boolean)
        {
            Editable = true;
        }
        ...
        field(50226; "Balance Account No."; Code[20])
        {
        }
        field(50227; "Ref. InterPost Doc. No."; Code[20])
        {
        }
        field(50228; "Refund Amount"; Decimal)
        {
            ...
            trigger OnValidate()
            var
                NewAppPEntry_1: Record "NewApplication Payment Entry";
            begin

                ConfOrdernew.RESET;
                IF ConfOrdernew.GET("Document No.") THEN
                    ConfOrdernew.TESTFIELD("Application Closed", FALSE);
        ...
        field(50229; Migration; Boolean)
        {
        }
        field(50230; "BarCode No."; Code[20])
        {
            Editable = false;
        }
        ...
        field(50242; "App Data"; Boolean)
        {
            Editable = false;
        }
        field(50243; "App Transfer in LLPS"; Boolean)
        {
            Editable = false;
        }
        ...
        field(50245; "Export Data"; Boolean)
        {
            DataClassification = ToBeClassified;
        }
        field(50246; "Send Mail"; Boolean)
        {
            DataClassification = ToBeClassified;
            Editable = false;
        }
        field(50247; "E-Mail Send Error"; Boolean)
        {
            DataClassification = ToBeClassified;
            Editable = false;
        }
        field(50248; "Online Customer Receipt"; Boolean)
        {
            DataClassification = ToBeClassified;
            Editable = false;
        }
        field(50249; "PDF Created"; Boolean)
        {
            DataClassification = ToBeClassified;
            Editable = false;
        }
        field(50250; "Receipt Time"; Time)
        {
            DataClassification = ToBeClassified;
            Editable = false;
        }
    ...
    keys
    {
        ...
        key(Key16; "Posted Document No.", "Receipt post on InterComp", "Create from MSC Company", "Cheque Status")
        {
        }
        ...
        key(Key18; "Receipt post on InterComp")
        {
        }
        key(Key19; "Document Type", "Posted Document No.", "Payment Mode", "BRS Created All Comp From MSC")
        {
        }
        key(Key20; "Create from MSC Company", "MJV Entry Not Posted")
        {
        }
        key(Key21; "Document No.", Posted, "Posting date", "Payment Mode", "Cheque Status")
        {
            SumIndexFields = Amount;
        }
        key(Key22; "Document No.", "Posting date", "Cheque Status")
        {
            SumIndexFields = Amount;
        }
    }
    ...
    begin
        ...
        UserSetup.TESTFIELD("Responsibility Center");  //300724
        "User Branch Code" := UserSetup."Responsibility Center";  //090921
    ...
    var
        ...
        BPayEntry: Record "NewApplication Payment Entry";
        ...
        LastLineNo: Integer;
        InsertAppLines: Record "NewApplication Payment Entry";
    ...
    var
        BondSetup: Record "Unit Setup";
        PaymentLines: Record "NewApplication Payment Entry";
        CashAmount: Decimal;
    begin
    ...
    var
        PmtLines: Record "NewApplication Payment Entry";
    begin
        IF "Document Type" <> "Document Type"::Application THEN
            EXIT;
    ...
    begin
        ...
        //IF NODNOCHdr.GET(NODNOCHdr.Type::Vendor, VendCode) THEN;//Need to check the code in UAT
