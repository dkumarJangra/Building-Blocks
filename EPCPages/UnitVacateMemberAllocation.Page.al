page 98005 "Unit Vacate /Member Allocation"
{
    ...
    layout
    {
        area(content)
        {
            group("Confirmed application")
            {
                ...
                field("Send for Approval (Unit Vacte)"; Rec."Send for Approval (Unit Vacte)")
                {
                }
                field("Send for Aproval Dt (UnitVct)"; Rec."Send for Aproval Dt (UnitVct)")
                {
                }
                field("Approval Status (Unit Vacte)"; Rec."Approval Status (Unit Vacte)")
                {
                }
                field("Send for Approval (Member)"; Rec."Send for Approval (Member)")
                {
                }
                field("Send for Aproval Dt (Member)"; Rec."Send for Aproval Dt (Member)")
                {
                }
                field("Approval Status (Member)"; Rec."Approval Status (Member)")
                {
                }
            }
            ...
            part("Document Approval Details For Unit Vacate"; "Document Approval Details")
            {
                Caption = 'Document Approval Details For Unit Vacate';
                SubPageLink = "Document No." = FIELD("No."),
                              "Document Type" = CONST("Unit Vacate");
            }
            ...
            part("Document Approval Details For Member Allocation"; "Document Approval Details")
            {
                Caption = 'Document Approval Details For Member Allocation';
                SubPageLink = "Document No." = FIELD("No."),
                              "Document Type" = CONST("Member Allocation");
            }
    ...
    actions
    {
        area(navigation)
        {
            ...
            group("Function")
            {
                action("Send for Reassign Member")
                {
                    Caption = 'Send for Approval (For Member)';
                    ...
                    begin
                        ...
                        IF CONFIRM('Do  you want to send Document Send for Approval') THEN BEGIN
                            Rec.TESTFIELD("Send for Approval (Member)", FALSE);
                            LineNo := 0;
                            ...
                            ApprovalWorkflowforAuditPr.SETRANGE("Document Type", ApprovalWorkflowforAuditPr."Document Type"::"Member Allocation");
                            ...
                            IF ApprovalWorkflowforAuditPr.FINDSET THEN BEGIN
                                REPEAT
                                    ...
                                    RequesttoApproveDocuments."Document Type" := RequesttoApproveDocuments."Document Type"::"Member Allocation";
                              ...
                              UNTIL ApprovalWorkflowforAuditPr.NEXT = 0;
                                Rec."Send for Approval (Member)" := TRUE;
                                Rec."Send for Aproval Dt (Member)" := TODAY;
                                Rec."Approval Status (Member)" := Rec."Approval Status (Member)"::" ";
                                Rec.MODIFY;
                            END ELSE
                ...
                action("Reassign Member")
                {
                    ...
                    begin
                        ...
                        AccessControl.SETRANGE("Role ID", 'A_CUSTOMERCHANGE');
                        IF NOT AccessControl.FINDFIRST THEN
                            ERROR('You do not have permission of role : A_CUSTOMERCHANGE');
                        //ALLECK 060313 End

                        IF CONFIRM('Do you want Reassigned new Member?', FALSE) THEN BEGIN
                            //ALLECK 020313 START
                            //AmountToWords.InitTextVariable;
                            //AmountToWords.FormatNoText(AmountText1,ApplicationForm.CheckPaymentAmount("Application No."),'');

                            //IF CONFIRM(STRSUBSTNO(Text014,ApplicationForm.CheckPaymentAmount("Application No."),"Customer No.","New Member",
                            //   FIELDCAPTION("Application No."),"Application No.","Posting Date")) THEN BEGIN
                            IF CONFIRM(Text006) THEN BEGIN //ALLECK020313
                                ...
                                IF AppPaymentEntry.FINDFIRST THEN
                                    ERROR('First Cleared Check before Reassigned the Member.');

                                IF Rec."Customer No." = Rec."New Member" THEN
                                    ERROR('New Member No. must be differ to existing Member No.');
                                Rec.CALCFIELDS("Amount Received");
                                Rec.CALCFIELDS("Total Received Amount");
                                Rec.CALCFIELDS("Amount Refunded");

                                UnitpayEntry.RESET;
                                UnitpayEntry.SETRANGE("Document No.", Rec."No.");
                                UnitpayEntry.SETFILTER("Charge Code", '=%1', 'BSP2');
                                IF NOT UnitpayEntry.FINDFIRST THEN BEGIN
                                    Rec.TESTFIELD("New Member");
                                    ArchiveConfirmedOrder.RESET;
                                    ArchiveConfirmedOrder.SETRANGE(ArchiveConfirmedOrder."No.", Rec."No.");
                                    ...
                                    OldCust := '';
                                    OldCust := Rec."Customer No.";
                                    IF OldCust <> Rec."New Member" THEN BEGIN
                                        TransferMemberAmount;
                                        MESSAGE('Member to Member successfully transfer');
                                    END;
                                END ELSE
                                    MESSAGE('You can not change customer');
                            END;
                            Rec."New Member" := '';
                            Rec.MODIFY;
                        END;//ALLECK 030313
                ...
                {
                    Caption = 'Send for Approval (For Unit Vacate)';
                    ...
                    begin
                        ...
                        Rec."Send for Approval (Unit Vacte)" := False;
                        Rec.MODIFY;
                        //Code added 24102025 END
                        Commit;


                        IF CONFIRM('Do  you want to send Document Send for Approval') THEN BEGIN
                            Rec.TESTFIELD("Send for Approval (Unit Vacte)", FALSE);
                            LineNo := 0;
                            RequesttoApproveDocuments.RESET;
                            ...
                            ApprovalWorkflowforAuditPr.RESET;
                            ApprovalWorkflowforAuditPr.SETRANGE("Document Type", ApprovalWorkflowforAuditPr."Document Type"::"Unit Vacate");
                            ApprovalWorkflowforAuditPr.SETRANGE("Requester ID", USERID);
                            IF ApprovalWorkflowforAuditPr.FINDSET THEN BEGIN
                                REPEAT
                                    RequesttoApproveDocuments.RESET;
                                    RequesttoApproveDocuments.INIT;
                                    RequesttoApproveDocuments."Document Type" := RequesttoApproveDocuments."Document Type"::"Unit Vacate";
                                    RequesttoApproveDocuments."Document No." := Rec."No.";
                                    //RequesttoApproveDocuments."Document Line No." := "Line No.";
                                    RequesttoApproveDocuments."Line No." := LineNo + 10000;
                                    RequesttoApproveDocuments.Amount := Rec.Amount;
                                    ...
                                    LineNo := RequesttoApproveDocuments."Line No."
                              UNTIL ApprovalWorkflowforAuditPr.NEXT = 0;
                                Rec."Send for Approval (Unit Vacte)" := TRUE;
                                Rec."Send for Aproval Dt (UnitVct)" := TODAY;
                                IF Rec."Approval Status (Unit Vacte)" = Rec."Approval Status (Unit Vacte)"::Rejected THEN
                                    Rec."Approval Status (Unit Vacte)" := Rec."Approval Status (Unit Vacte)"::" ";
                                Rec.MODIFY;
                            END ELSE
                                ERROR('Approver not found against this Sender');
                        END ELSE
                ...
                action("Archive & Unit Vacate")
                {
                    ...
                    begin
                        ...
                        CLEAR(AutoPlotvacate);
                        NewConfOrder_1.RESET;
                        NewConfOrder_1.SETRANGE("No.", Rec."No.");
                        IF NewConfOrder_1.FINDFIRST THEN BEGIN
                            AutoPlotvacate.ReportFilters(Rec."No.", TRUE);
                            AutoPlotvacate.RUNMODAL;
                            //HK
                            ConfirmedOrder_5.RESET;
                            IF ConfirmedOrder_5.GET(Rec."No.") THEN BEGIN
                                ConfirmedOrder_5."Approval Status (Unit Allot)" := ConfirmedOrder_5."Approval Status (Unit Allot)"::" ";
                                ConfirmedOrder_5."Send for Approval (Unit Allot)" := FALSE;
                                ConfirmedOrder_5."Send for Aproval Dt (UnitAllt)" := 0D;
                                ConfirmedOrder_5.MODIFY;
                            END;
                            //
    ...
    begin
        ...
        UnitLifeCycle."Unit Vacate Date" := TODAY;
        UnitLifeCycle."Unit Vacate Time" := TIME;
        UnitLifeCycle."Unit Payment Plan" := Rec."Unit Payment Plan";
        UnitLifeCycle."Unit Cost" := OldUnitLifeCycle."Unit Cost";
        UnitLifeCycle."Application Unit Cost" := Rec.Amount;
        UnitLifeCycle."Type of Transaction" := UnitLifeCycle."Type of Transaction"::"Unit Vacate";
