page 97997 "Posted Payable Form"
{
    // BBG2.01 070215 this code is used for update entries in MScompany
    ...
    layout
    {
        area(content)
        {
            group(General)
            {
                ...
                field("Invoice Reversed"; Rec."Invoice Reversed")
                {
                }
    ...
    actions
    {
        area(navigation)
        {
            group("F&unction")
            {
                ...
                action("&Get Lines")
                {
                    ...
                    begin
                        ...
                        END ELSE
                            IF Rec.Type = Rec.Type::Commission THEN BEGIN
            ...
            group("&Reverse")
            {
                ...
                {
                    Caption = 'Print Posted Voucher';
                    Visible = false;

                    trigger OnAction()
                    begin
                        ASSVoucher := TRUE;
                        Rec.CALCFIELDS("Posted Document No.");
                        GLEntry.RESET;
                ...
                action("Reverse Invoice")
                {
                    ...
                    begin
                        IF USERID <> '1005' THEN BEGIN
                            ...
                            AssPmtHdr.SETRANGE(AssPmtHdr."Reversal done in LLP Companies", FALSE);
                            AssPmtHdr.SETRANGE("Document No.", Rec."Pmt from MS Company Ref. No.");
                            IF AssPmtHdr.FINDFIRST THEN
                                REPEAT
                                    AssPmtHdr.TESTFIELD("Payment Reversed", TRUE);
                                UNTIL AssPmtHdr.NEXT = 0;
                        END;



                        Rec.TESTFIELD("Payment Reversed", FALSE);  //BBG2.01 070215
                        IF (Rec.Type IN [Rec.Type::Incentive]) THEN
                        ...
                        IF VLE.FINDFIRST THEN BEGIN
                            ...
                            RevEntry.DELETEALL;
                            //ReversalEntry.AutoReverseTransaction(VLE."Transaction No.","Posting Date");
                            PAGERevEntries.RUN;
                        END;
                        ...
                        AssPmtHdr.RESET;
                        AssPmtHdr.SETRANGE("Company Name", COMPANYNAME);
                        AssPmtHdr.SETRANGE(AssPmtHdr."Payment Reversed", TRUE);
                        AssPmtHdr.SETRANGE(AssPmtHdr."Reversal done in LLP Companies", FALSE);
                        AssPmtHdr.SETRANGE("Document No.", Rec."Pmt from MS Company Ref. No.");
                        IF AssPmtHdr.FINDFIRST THEN
                            REPEAT
                                AssPmtHdr."Inv Reversed in LLP Companies" := TRUE;
                                AssPmtHdr.MODIFY;
                            UNTIL AssPmtHdr.NEXT = 0;
                    ...
                    begin
                        IF USERID <> '1005' THEN BEGIN
                            ...
                            IF AssPmtHdr.FINDFIRST THEN
                                ...
                                UNTIL AssPmtHdr.NEXT = 0;
                        END;
                        Rec.TESTFIELD("Invoice Reversed");   //BBG2.01 070215
                        ...
                        IF AssPmtHdr.FINDFIRST THEN
                            REPEAT
                                AssPmtHdr."Inv Reversed in LLP Companies" := TRUE;
                                AssPmtHdr."Pmt. Reversed in LLPCompanies" := TRUE;
                                AssPmtHdr."Reversal done in LLP Companies" := TRUE;
                                AssPmtHdr.MODIFY;
                            UNTIL AssPmtHdr.NEXT = 0;
                    end;
                }
                group("Special Incentive")
                {
                    Caption = 'Special Incentive';
                    action("Reverse &Invoice")
                    {
                        Caption = 'Reverse &Invoice';
                        ...
                        begin
                            ...
                            IF (Rec.Type IN [Rec.Type::Incentive]) AND (Rec."Sub Type" = Rec."Sub Type"::Direct) THEN BEGIN
                                DocNo_2 := '';
                                VLE.RESET;
                                VLE.SETCURRENTKEY("Vendor No.");
                                VLE.SETRANGE("Vendor No.", Rec."Paid To");
                                DocNo_2 := Rec."Document No." + '_1';
                                VLE.SETRANGE("External Document No.", DocNo_2);
                                VLE.SETRANGE("Document Type", VLE."Document Type"::Invoice);
                                IF VLE.FINDFIRST THEN BEGIN
                                    CLEAR(ReversalEntry);
                                    VLE.CALCFIELDS("Remaining Amount");
                                    IF VLE."Remaining Amount" = 0 THEN
                                        VendEntryApplyPostedEntries.UnApplyVendLedgEntry(VLE."Entry No.");
                                    IF VLE.Reversed THEN
                                        ReversalEntry.AlreadyReversedEntry(Rec.TABLECAPTION, VLE."Entry No.");
                                    VLE.TESTFIELD("Transaction No.");
                                    COMMIT;
                                    ...
                                    //PAGERevEntries.RUN;
                                    //    COMMIT;
                                    ReversalEntry.ReverseTransaction(VLE."Transaction No."); //

                                END;
                                Commit;
                                //060325 for Special incentive Club 9 reversal Start
                                DocNo_2 := '';
                                VLE.RESET;
                                VLE.SETCURRENTKEY("Vendor No.");
                                VLE.SETRANGE("Vendor No.", Rec."Paid To");
                                DocNo_2 := Rec."Document No." + '_1';
                                VLE.SETRANGE("External Document No.", DocNo_2);
                                VLE.SETRANGE("Document Type", VLE."Document Type"::" ");
                                IF VLE.FINDFIRST THEN BEGIN
                                    CLEAR(ReversalEntry);
                                    VLE.CALCFIELDS("Remaining Amount");
                                    IF VLE."Remaining Amount" = 0 THEN
                                ...
                                END;
                                //060325 for Special incentive Club 9 reversal END
                            END ELSE
                                ERROR('Only Special Incentive Reversal option is available');
                        end;
                    }
                    action("Reverse &Payment")
                    {
