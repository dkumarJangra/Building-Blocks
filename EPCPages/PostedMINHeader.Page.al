page 97859 "Posted MIN Header"
{
    ...
    actions
    {
        area(processing)
        {
            action("Print &Material Issue Note")
            {
                Caption = 'Print &Material Issue Note';
                Promoted = true;
            ...
            action("Print &Debit Note")
            {
                ...
                begin
                    ...
                    GatePassHdr.SETRANGE(GatePassHdr."Document No.", Rec."Document No.");
                    IF GatePassHdr.FIND('-') THEN BEGIN
                        REPORT.RUN(97749, TRUE, FALSE, GatePassHdr);
                    END;
                end;
            }
            action(NAVIGATE)
    ...
    begin
        ...
        IF UserMgt.GetPurchasesFilter() <> '' THEN BEGIN
            Rec.FILTERGROUP(2);
            Rec.SETRANGE("Shortcut Dimension 1 Code", UserMgt.GetPurchasesFilter());
            Rec.FILTERGROUP(0);
        END;
    end;
